...
async fn main() -> Result<(), DbErr> {
    ...
    if let Some(conn) = db {
        match conn {
            DatabaseConnection::SqlxPostgresPoolConnection(_) => info!("database connected!"),
            DatabaseConnection::Disconnected => info!("database disconnected!"),
        }
    }
    let cors = warp::cors()
        .allow_any_origin()
